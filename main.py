...
def save_watermarked_image():
    ...
    try:
        # 1. Open original image and convert to RGBA for transparency support
        base_image = Image.open(uploaded_file_path).convert("RGBA")

        # 2. Create a blank, completely transparent overlay layer for the text
        txt_layer = Image.new("RGBA", base_image.size, (255, 255, 255, 0))
        draw = ImageDraw.Draw(txt_layer)

        # 3. Dynamic Font Scaling: Scales the font size relative to image width
        ...
        # 4. Determine color contrast based on user's selection (50% opacity)
        if color_mode.get() == "dark":
            text_color = (0, 0, 0, 128)  # Semi-transparent Black
        else:
            text_color = (255, 255, 255, 128)  # Semi-transparent White

        # 5. Position the text exactly in the center of the canvas
        ...
        # 6. Draw text onto the transparent overlay layer
        draw.text((x_position, y_position), text_to_add, fill=text_color, font=font)

        # 7. Composite the layers together and drop the alpha channel for saving
        combined_image = Image.alpha_composite(base_image, txt_layer)
        final_image = combined_image.convert("RGB")

        # 8. Extract original details to format the default save filename
...
window.config(padx=40, pady=40, bg="#f5f5f7")  # Clean minimalist background color
...
# --- Section 2: Watermark Text Input ---
...
# --- Section 3: Contrast Toggle Selection ---
